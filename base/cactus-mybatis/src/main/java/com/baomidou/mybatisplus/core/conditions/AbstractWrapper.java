 ...
 * @since 2017-05-26
 * 新增isNUllByStrGetCondition、isNUllByCollectionGetCondition用于将空值的条件排除掉
 * @update 2021-10-15
 */
...
public abstract class AbstractWrapper<T, R, Children extends AbstractWrapper<T, R, Children>> extends Wrapper<T>
    ...
    public Children between(boolean condition, R column, Object val1, Object val2) {
        //确保两个值都不为空
        condition = isNUllByStrGetCondition(val1,condition);
        condition = isNUllByStrGetCondition(val2,condition);
    ...
    public Children in(boolean condition, R column, Collection<?> coll) {
        condition = isNUllByCollectionGetCondition(condition,coll);
    ...
    public Children in(boolean condition, R column, Object... values) {
        condition = isNUllByCollectionGetCondition(condition,values);
    ...
    protected Children likeValue(boolean condition, SqlKeyword keyword, R column, Object val, SqlLike sqlLike) {
        condition = isNUllByStrGetCondition(val,condition);
    ...
    /**
     * 如果值为null或者空串，条件修改为false
     * @author lht
     * @since  2021/5/11 16:43
     * @param  val 值
     * @param  condition 当前条件
     */
    private boolean isNUllByStrGetCondition(Object val, boolean condition){
        //如果没有开启忽略空值，则直接返回条件
        if(!getIgnoreNull()){
            return condition;
        }
    ...
    /**
     * 如果值为null或者空集合，条件修改为false
     * @author lht
     * @since  2021/5/11 17:10
     * @param  condition 当前条件
     * @param  val 集合
     */
    private boolean isNUllByCollectionGetCondition(boolean condition,Collection<?> val){
        //如果没有开启忽略空值，则直接返回条件
        if(!getIgnoreNull()){
            return condition;
        }
     ...
     * @param  val 集合
     */
    private boolean isNUllByCollectionGetCondition(boolean condition,Object... val){
        //如果没有开启忽略空值，则直接返回条件
        if(!getIgnoreNull()){
            return condition;
        }
